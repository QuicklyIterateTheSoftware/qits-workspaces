 ...
 * publish); the test fake resolves it to {@code 127.0.0.1} and the port a host-clone service bound.
 *
 * <p>Because the target is the actual container port (not an ephemeral host port frozen at {@code
 * docker run}), a service that gains a web-view port after its container exists is reachable

...
public class LogLevelClassifier implements LogClassifier {
  ...
  public Optional<Classification> classify(String logBatch) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ServiceEventSeverity severity = classifyLine(lines[i]);
      if (severity == ServiceEventSeverity.ERROR) {
        return Optional.of(classification(ServiceEventSeverity.ERROR, lines[i], i));
      }
      if (severity == ServiceEventSeverity.WARNING && firstWarning < 0) {
        firstWarning = i;
      }
    }
    if (firstWarning >= 0) {
      return Optional.of(
          classification(ServiceEventSeverity.WARNING, lines[firstWarning], firstWarning));
    }
  ...
  private static ServiceEventSeverity classifyLine(String line) {
    ...
    if (ERROR_SIGNAL.matcher(line).find()) {
      return ServiceEventSeverity.ERROR;
    }
    if (WARNING_SIGNAL.matcher(line).find()) {
      return ServiceEventSeverity.WARNING;
    }
    return null;
  }

  /** Maps a declared level token to a finding severity; INFO and below are not findings. */
  private static ServiceEventSeverity levelSeverity(String token) {
    return switch (token) {
      case "ERROR", "SEVERE", "FATAL", "CRITICAL", "PANIC" -> ServiceEventSeverity.ERROR;
      case "WARN", "WARNING", "NOTICE" -> ServiceEventSeverity.WARNING;
      default -> null;
    };
  }

  private static Classification classification(
      ServiceEventSeverity severity, String line, int offset) {
    Matcher exception = EXCEPTION_NAME.matcher(line);
    String errorType =
        exception.find()
            ? exception.group(1)
            : (severity == ServiceEventSeverity.ERROR ? "error-log" : "warning-log");

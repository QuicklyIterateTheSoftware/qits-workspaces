...
/**
 * {@link PushCausation} over the event bus's ambient cause — this service's producer half of the
 * causation chain, and the counterpart to {@link SCMReleaseAnnouncer}'s publishing half.
 *
 * <p>It lives in {@code bus/} for the reason that class does: the port is in {@code
 * domain/…/control}, the implementation is where the bus is, and {@code domain} stays free of {@code
 ...
 *       establishes the scope for the resource method, so a release triggered as part of a larger
 *       chain pushes under that chain's cause with nothing said here;
 ...
 * one bean covers a release's branch and tag, a promotion, an integrate, a branch create and a
 * cleanup alike. There is no external remote here to leak an internal identifier to.
 */
@ApplicationScoped
